...
function AddUser({ isOpen, onClose }) {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/api/users/${user.role === 'Doctor' ? 'patients' : 'doctors'}`);
                setUsers(response.data);
            } catch (error) {
                toast.error(`Error fetching ${user.role === 'Doctor' ? 'patients' : 'doctors'}: ` + error.message);
            }
        };
        if (isOpen) {
            fetchUsers();
    ...
    const handleAddUser = async () => {
        if (selectedUser) {
            ...
            try {
                await axios.post(
                    `http://localhost:3000/api/users/request-permission/${selectedUser._id}`,
                    {},
                    { withCredentials: true }
                );
                toast.success(`Successfully sent request to ${user.role === 'Doctor' ? 'Dr. ' : ''}${selectedUser.firstName} ${selectedUser.lastName}`);
                onClose();
            } catch (error) {
                            ...
                            {filteredUsers.map(user => (
                                <li 
                                    ...
                                    className={`px-4 py-2 cursor-pointer hover:bg-gray-100 
                                        ${selectedUser?._id === user._id ? 'bg-[#30336B] text-white' : ''}`}
                                >
                    ...
                    <div className="mb-4 p-3 bg-gray-50 rounded-md flex justify-between items-center">
                        <div>
                            <p className="text-[#30336B] font-semibold">
                                Selected: {user.role === 'Doctor' ? 'Dr. ' : ''}{selectedUser.firstName} {selectedUser.lastName}
                            </p>
